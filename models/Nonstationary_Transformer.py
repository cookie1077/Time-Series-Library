...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        #print(x_enc.shape, "is the shape of x_enc")
        x_dec_new = torch.cat([x_enc[:, -self.label_len:, :], torch.zeros_like(x_enc[:, -self.pred_len:, :])], dim=1).to(x_enc.device).clone()
        #x_dec_new = torch.cat([x_enc, torch.zeros_like(x_dec.unsqueeze(1))], dim=1).to(x_enc.device).clone()
        #x_dec_new = x_enc.clone()
